...
class UDA_adas_trainer(Base_trainer):
    def __init__(self, config):
        ...
        self.seg_loss_weight = self.config.initial_seg_loss_weight

        self.loss_increment = float((self.config.target_seg_loss_weight[1] - self.config.initial_seg_loss_weight[1]) / self.config.target_seg_loss_iteration)
        ...
        self.label_filter = self.init_label_filter_model(None)
    ...
    def train(self) -> None:  
        ...
        for iteration in tqdm(range(1, self.config.max_iteration), desc=self.config.ex+'| Training'):
            ...
            self.seg_model.train()
            ### learning cvt imgs
            output_s = self.seg_model(batch['S_t']['img'])
            output_s = F.interpolate(output_s, batch['S_t']['label'].size()[1:], mode='bilinear', align_corners=False)
            loss_seg = self.seg_loss_set.CrossEntropy2d(output_s, batch['S_t']['label']) * self.seg_loss_weight[0]
            ...
            if iteration % self.config.tensor_interval == 0:
                output_s = F.interpolate(output_s, batch['S_t']['label'].size()[1:], mode='bilinear', align_corners=False)
                prediction_s = torch.argmax(output_s, dim = 1)
                self.plot_tensor_img(prediction_s2t=prediction_s, batch=batch, d_recons=None, id_recon=None, cvt_imgs=None, pd_label=pd_label, iteration=iteration)

                loss_sum = dict()
                loss_sum['Seg'] = loss_seg
                self.plot_tensor_loss(loss_sum, loss_dict={}, iteration=iteration)
